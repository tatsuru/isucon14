...
(
  ...
  created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '登録日時',
  PRIMARY KEY (id),
  INDEX chair_id_created_at (chair_id, created_at)
)
...
(
  ...
  INDEX chair_id_updated_at (chair_id, updated_at)
)
  COMMENT = 'ライド情報テーブル';
...
(
  ...
  PRIMARY KEY (id),
  INDEX ride_id_app_sent_at_created_at (ride_id, app_sent_at, created_at),
  INDEX ride_id_created_at (ride_id, created_at),
  INDEX ride_id_chair_sent_at_created_at (ride_id, chair_sent_at, created_at),
  INDEX status_chair_id_updated_at (status, ride_id, created_at)
)
...
(
  ...
  INDEX user_id_used_by_created_at (user_id, used_by, created_at),
  INDEX used_by (used_by),
  INDEX user_id_code_used_by (user_id, code, used_by)
)
  COMMENT 'クーポンテーブル';
